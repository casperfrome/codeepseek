...
func (s *Server) resolveModelOrFallback(modelName string) (*provider.ResolvedRoute, error) {
	if pm := s.activeProviderManager(); pm != nil {
		route, err := pm.ResolveModel(modelName)
		if err == nil {
			return route, nil
		}
		// Codex's plan mode (and other aux flows) request a helper model such as
		// "gpt-5.4-mini" that has no route here. Fall back to the configured default
		// model so these requests still route through the bridge instead of 404ing.
		if alias := s.defaultModelAlias(); alias != "" && alias != modelName {
			if fb, fbErr := pm.ResolveModel(alias); fbErr == nil {
				logger.L().Info("未知模型回退到默认模型", "requested", modelName, "fallback", alias)
				return fb, nil
			}
		}
		return nil, err
...
// defaultModelAlias returns the configured default route alias (e.g. "moonbridge"),
// used to absorb requests for unknown helper models.
func (s *Server) defaultModelAlias() string {
	if snap := s.runtimeSnapshot(); snap != nil {
		return snap.Config.DefaultModelAlias()
	}
	return ""
}
