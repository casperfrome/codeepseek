...
func (r *Registry) disableAllConsumers(reason error) {
	// reason is always ErrNoProvider here (see Init): persistence is simply not
	// configured, which is an expected, benign state — log it informationally.
	r.logInfo("持久化未配置：消费者以内存模式运行 (no persistence provider registered)",
...
// logInfo is a nil-safe logger wrapper.
